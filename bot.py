...
📸 Подписи к фото — просто отправь фото!
...
async def activate_sub(message: Message):
    if message.from_user.id != OWNER_ID:
        return
    parts = message.text.split()
    if len(parts) != 3:
        await message.answer("Формат: /activate user_id plan\nПример: /activate 123456 pg_pro")
        return
    user_id = int(parts[1])
    plan = parts[2]
    set_subscription(user_id, plan, 30)
...
async def caption(callback: CallbackQuery):
    ...
    await callback.message.answer(
        "📸 Подпись к фото\n\nОтправь фото — я сам его проанализирую и напишу подпись!",
        reply_markup=back_menu()
    )
    await callback.answer()
